...
{
   OLED_WR_Byte(0xb0+y,1);           //设置页地址
	 OLED_WR_Byte(0x0f&x,1);           //设置列地址的低四位
	 OLED_WR_Byte((0xf0&x)>>4|0x10,1); //设置列地址的高四位
}
...
void OLED_WriteCN(uint8_t x,uint8_t y,uint8_t m,uint8_t count)
{
   uint8_t i,j = 0;
	 while(count--)
	 {
		   ...
		   OLED_SetPos(x,y);
			 for(i=0;i<16;i++)
				 OLED_WR_Byte(F16x16[m][i+j*32],0);
			 OLED_SetPos(x,++y);
			 for(i=0;i<16;i++)
				 OLED_WR_Byte(F16x16[m][i+16+j*32],0);
		   x += 16;
   		 OLED_SetPos(x,--y);
		   j++;
	 }
