...
extern void main_interface(void);  //功能主界面
extern void main1_interface(void);  //功能主界面
extern u8 lock;
//extern u8 alarmFlag;//是否报警的标志
extern u8 Beep_Status; //蜂鸣器报警标志位
//extern u8 alarm_is_free;  {"target":"LED","value":0}

u8 alarmFlag;//是否报警的标志
u8 alarm_is_free;
extern u8 Power_Status;
extern u8 upload_card_number_flag; //发送卡号给小程序标志
extern u8 upload_fin_number_flag;//发送指纹个数给小程序标志
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				else
				{
					...
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)Beep_Status = 1;//打开报警器 BEEP_ON  alarmFlag = 1
						else Beep_Status = 0;//关闭报警器 alarmFlag = 0
					...
					else if(strstr(json_value->valuestring,"Addfin") != NULL) //增加指纹
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)
						{
					...
					else if(strstr(json_value->valuestring,"Rmfin") != NULL) //增加指纹
					{
						json_value = cJSON_GetObjectItem(json,"value");
						if(json_value->valueint)
						{
					...
					else if(strstr(json_value->valuestring,"Addcard") != NULL) //增加卡片
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)
						{
					...
					else if(strstr(json_value->valuestring,"Rmcard") != NULL) //删除卡片
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)
						{
					...
					else if(strstr(json_value->valuestring,"Power") != NULL) //关闭开启门禁电源
					{
						json_value = cJSON_GetObjectItem(json,"value");
					
					
						if(json_value->valueint) // 开启电源
						{
					...
					else if(strstr(json_value->valuestring,"Get_card") != NULL) //发送卡号到小程序
					{
						json_value = cJSON_GetObjectItem(json,"value");
					
					
						if(json_value->valueint) // 调用发送函数
						{
							DEBUG_LOG("发送卡号");
					...
					else if(strstr(json_value->valuestring,"Get_fin") != NULL) //发送指纹个数到小程序
					{
						json_value = cJSON_GetObjectItem(json,"value");
