...
u8 PS_ValidTempleteNum(u16 *ValidN)
{
  ...
  data=JudgeStr(2000);
	...
	if(ensure==0x00)
	{
		DEBUG_LOG("有效指纹个数=%d",*ValidN);
		//fingerprints_num = (data[10]<<8)+data[11];
	}
	else
		DEBUG_LOG("\r\n%s",EnsureMessage(ensure));
	return ensure;
...
u8 PS_HandShake(u32 *PS_Addr)
{
	...
	MYUSART_SendData(0X00);	
	delay_ms(200);
	DEBUG_LOG("%d  %x %x %x\n",USART3_RX_STA,USART3_RX_BUF[0],USART3_RX_BUF[1],USART3_RX_BUF[6]);
