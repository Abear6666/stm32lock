...
{   
    ...
    while ((GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)==1)&&retry<100)//DHT11会拉低40~80us
	{
		retry++;
		delay_us(1);
	};	 
    ...
    while ((GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)==0)&&retry<100)//DHT11拉低后会再次拉高40~80us
	{
		retry++;
		delay_us(1);
	};
...
{
 	u8 retry=0;
	while((GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)==1)&&retry<100)//等待变为低电平
	{
		retry++;
		delay_us(1);
	}
	retry=0;
	while((GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_0)==0)&&retry<100)//等待变高电平
	{
		retry++;
		delay_us(1);
	}
